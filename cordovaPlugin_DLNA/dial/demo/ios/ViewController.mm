...
- (IBAction)start:(id)sender {
  ...
  Result res = mr->set_av_transport_uri("http://huace.cdn.ottcloud.tv/huace/videos/dst/2016/08/14461-ai-de-zhui-zong-01-ji_201608180551/14461-ai-de-zhui-zong-01-ji.m3u8");
    
    ATRACE("code: %d", res.code);
  });
}
...
- (IBAction)seek:(id)sender {
  ...
  for (int i = 0; i < ls.size(); i++) {
    MediaRenderer* mr = ls[i]->as_media_renderer();
    if ( mr ) {
      Result r = mr->seek(1000);
      ATRACE("%s", r.to_json_string().c_str());
    }
  }
    ...
    if ( mr ) {
      ...
      if ( res2.code == 0 ) {
        data = [data stringByAppendingFormat:@"##MediaDuration: %d, RelTime: %d, TrackURI: %s",
                ...
                res2.data.HasMember("TrackDuration") ? res2.data["TrackDuration"].GetInt() : 0,
                res2.data.HasMember("RelTime") ? res2.data["RelTime"].GetInt() : 0,
                res2.data.HasMember("TrackURI") ? res2.data["TrackURI"].GetString() : ""
        ];
      }
    }
  }
