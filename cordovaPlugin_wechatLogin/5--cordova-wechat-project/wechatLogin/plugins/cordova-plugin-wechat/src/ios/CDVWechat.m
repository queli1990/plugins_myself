...
- (void)sendAuthRequest:(CDVInvokedUrlCommand *)command
{
    ...
    {
        // save the callback id
        self.loginId = command.callbackId;
    }
    else
    {
        [self failWithCallbackID:command.callbackId withMessage:@"发送请求失败"];
...
- (void)onResp:(BaseResp *)resp
{
    ...
    if (success)
    {
        if ([resp isKindOfClass:[SendAuthResp class]])
        {
            ...
            AFRequest *obj = [[AFRequest alloc] init];
            NSDictionary *dic = @{@"appid":@"wx8c74f72241cd32bc",@"secret":@"47ff7884d90768fab20b282c60000b37",@"code":authResp.code};
            
            __block __typeof(self) weakSelf = self;
            
            [obj requestDataWithParam:dic andBackResult:^(NSDictionary *dic) {
                
                CDVPluginResult *commandResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:dic];
                
                [weakSelf.commandDelegate sendPluginResult:commandResult callbackId:weakSelf.loginId];
            }];
        }
        else
        {
